...
# Regex pattern
pattern = re.compile(
    r"\[(\d{1,2}/\d{1,2}/\d{2,4}), (\d{1,2}:\d{2}(?::\d{2})? (?:AM|PM|am|pm))\] (.*?): (.*)"
)
...
if uploaded_file is not None:
    try:
        ...
        try:
            ...
            text = text.decode("utf-16")  # Fallback for WhatsApp export issues
        ...
        if df.empty:
            ...
            if not last_week_df.empty:
                st.markdown(f"### 🗓️ Week Period WORKDAY TIMESHEET ({last_monday.strftime('%b %d')} - {last_sunday.strftime('%b %d')})")
                ...
                st.warning("⚠️ No work hours found for the most recent week in the data.")
