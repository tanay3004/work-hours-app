...
# Time formats and regex patterns for both iOS and Android formats
patterns = [
    (re.compile(r"\[(\d{1,2}/\d{1,2}/\d{2,4}), (\d{1,2}:\d{2}:\d{2})\u202f?(AM|PM)\] (.*?): (.*)"), "%m/%d/%y %I:%M:%S %p"),
    (re.compile(r"(\d{1,2}/\d{1,2}/\d{2,4}), (\d{1,2}:\d{2})\u202f?(AM|PM) - (.*?): (.*)"), "%m/%d/%y %I:%M %p")
]
